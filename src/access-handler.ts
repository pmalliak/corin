...
export async function handleAccessRequest(
	...
	if (request.method === "GET" && pathname === "/authorize") {
		...
		// Check if client is already approved â€” no approval form so no CSRF cookie to clear
	...
	if (request.method === "POST" && pathname === "/authorize") {
		try {
			...
			// Build redirect headers â€” use Headers to support multiple Set-Cookie values
	...
	return createApp(env, _ctx).fetch(request);
}
