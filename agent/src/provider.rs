//! Where game state comes from. The live and fixture providers sit behind one
//! trait, so the session loop never learns which one it is talking to.

use std::sync::atomic::{AtomicUsize, Ordering};

use crate::contract::{CurrentGameState, LeagueState, LiveApiState, Snapshot, StatusPayload};

pub trait GameDataProvider: Send + Sync {
    /// Everything known right now. Called once per heartbeat, so it must not block.
...
impl GameDataProvider for FixtureProvider {
    fn snapshot(&self) -> Snapshot {
        ...
        Snapshot::status_only(self.script[step.min(self.script.len() - 1)])
    }
...
mod tests {
    ...
    fn the_startup_sequence_walks_from_offline_to_in_game() {
        ...
        assert_eq!(provider.snapshot().status, StatusPayload::OFFLINE);
        assert_eq!(provider.snapshot().status.league, LeagueState::Running);
        assert_eq!(provider.snapshot().status.live_api, LiveApiState::Available);
        assert_eq!(provider.snapshot().status.current_game, CurrentGameState::Active);
    }
    ...
    fn the_last_step_holds_once_the_script_runs_out() {
        ...
        let settled = provider.snapshot().status;
